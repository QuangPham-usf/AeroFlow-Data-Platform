...
[Part 1 (Extract-Load)](https://github.com/MarkPhamm/skytrax_reviews_extract_load) scraped **~160,000+ rows across four review types** (airline / seat / lounge / airport) from AirlineQuality.com (Skytrax — **permanently closed**; historical archive only) and loaded them into Snowflake. This project picks up where that left off — transforming **airline** reviews (~117k in marts / Mode industry bar) into a star schema, with full CI/CD, Infrastructure as Code, and orchestration.
